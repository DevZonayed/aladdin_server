...
let BinanceService = class BinanceService {
    ...
    async checkBalance(apiKey, secretKey) {
        return await this.executeBinanceApiAction(apiKey, secretKey, async (binance, binanceTest) => {
            try {
                const originalApiPromise = (0, order_utils_1.safePromiseBuild)(binance.futuresBalance(), 'Original API');
                const testnetApiPromise = (0, order_utils_1.safePromiseBuild)(binanceTest.futuresBalance(), 'Testnet API');
    ...
    async handleOrderResultProcessing(response, strategy, orderDto) {
        try {
            ...
            const { userId, success, message, order, ratio, } = (0, order_utils_1.retrieveDataFromOrderResult)(result);
            if (!success) {
            ...
            if (existOrder.status) {
                ...
                let payload = (0, order_utils_1.buildNewOrderPayload)(success, userId, strategy, orderDto, order, ratio, message);
    ...
    async updateExistingOrder(existOrder, orderDto, order) {
        try {
            ...
            const updatePayload = (0, order_utils_1.computeOrderUpdateDetails)(orderData, orderDto, order);
    ...
    async generateFutureOrders(strategy, credentials, orderDto) {
        ...
        return await this.executeBinanceApiAction(apiKey, apiSecret, async (binance, binanceTest) => {
            try {
                ...
                let allOpenOrders = this.orderService.findAllOpenOrders(userId);
                let openPositionCountPromise = this.getBinanceAccountOrderCount(instance, isMaxPositionIncludeOpen);
                let [binanceBalanceRes, prevOrderRes, openPositionCount, allOpenOrdersRes,] = await Promise.all([
                    (0, order_utils_1.safePromiseBuild)(binanceBalance),
                    (0, order_utils_1.safePromiseBuild)(prevOrder),
                    (0, order_utils_1.safePromiseBuild)(openPositionCountPromise),
                    (0, order_utils_1.safePromiseBuild)(allOpenOrders),
                ]);
                if (!binanceBalanceRes.success || binanceBalanceRes?.result?.code) {
                    throw new Error('Binance Balance Fetch Failed ' +
                ...
                if (openPositionCount.success) {
                    ...
                    throw new Error('Open Position Count Fetch Failed ' +
                        openPositionCount.error?.message ||
                        openPositionCount?.result?.msg);
                }
                let orderBouncedPayload = {
                    prevOrderRes,
                ...
                let order = '';
                if ((signalType == BinanceEnum_1.SignalTypeEnum.NEW ||
                    signalType == BinanceEnum_1.SignalTypeEnum.RE_ENTRY) &&
                    side == BinanceEnum_1.PositionSideEnum.LONG) {
                    let type = signalType == BinanceEnum_1.SignalTypeEnum.RE_ENTRY
                        ? partialOrderType
                        : newOrderType;
                ...
                else if ((signalType == BinanceEnum_1.SignalTypeEnum.NEW ||
                    signalType == BinanceEnum_1.SignalTypeEnum.RE_ENTRY) &&
                    ...
                    throw new Error('Invalid Signal Type or order side, Sended Signal Type: ' +
                        signalType +
                        ' Order Side: ' +
                        side);
                }
                return (0, order_utils_1.generateFutureOrdersResponse)(userId, order, orderDto, strategy, true, ratio);
            }
            catch (error) {
                return (0, order_utils_1.generateFutureOrdersResponse)(userId, error, orderDto, strategy, false);
            }
        });
    }
    async processClosingOfOpenFutureOrders(instance, symbol, side, quantity = 0) {
        try {
            ...
            let orderIds = (0, order_utils_1.identifyOrdersForCancellation)(openOrders, quantity, side);
            ...
            if (orderIds.length) {
                try {
                    ...
                    const remainingQuantity = quantity - (0, order_utils_1.computeAllOrderOrderQuantity)(orders);
    ...
    handleOrderBounced(payload) {
        console.log("Order Bounced Function Called");
        let { prevOrderRes, signalType, strategy, side, openPositionCount, allOpenOrdersRes, maxLongEntry, maxShortEntry, symbol } = payload;
