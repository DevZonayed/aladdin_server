...
let GoogleAuthService = class GoogleAuthService extends (0, passport_1.PassportStrategy)(passport_google_oauth20_1.Strategy, 'google') {
    constructor(config, authService) {
        super({
            clientID: config.get(core_service_1.GOOGLE_CLIENT_ID),
            clientSecret: config.get(core_service_1.GOOGLE_CLIENT_SECRET),
            callbackURL: config.get(core_service_1.GOOGLE_AUTH_REDIRECT),
            scope: ['profile', 'email'],
        });
        this.config = config;
